...
use jni_sys::{jobject, JNI_TRUE, JNI_VERSION_1_2, JNI_VERSION_1_4};
...
impl<'local> JNIEnv<'local> {
    ...
    fn ensure_version(&self, version: i32) -> Result<()> {
    ...
    ///
    /// This does a null check, and checks that the JNI version is >= 1.2
    ///
    /// # Safety
    ///
    ...
    pub unsafe fn from_raw(ptr: *mut sys::JNIEnv) -> Result<Self> {
        ...
        if env.version() < JNI_VERSION_1_2 {
            Err(Error::UnsupportedVersion)
        } else {
            Ok(env)
        }
    ...
    /// Requires a JNI version >= 1.2
    unsafe fn from_raw_unchecked(ptr: *mut sys::JNIEnv) -> Self {
        JNIEnv {
            internal: ptr,
            lifetime: PhantomData,
        }
    ...
    // FIXME: the `JNIVersion` type is a bit of a mess and should
    // probably be reworked. In the meantime we have this alternative
    // that just returns the int JNI version
    fn version(&self) -> i32 {
        // Safety: GetVersion is 1.1 API that must be valid
        unsafe { jni_call_unchecked!(self, v1_1, GetVersion) }
    }

    // TODO: if we update `JNIVersion`, we should also rename this to `version`,
    // since it's more Rusty to omit the `get_` prefix from a getter function.
    ...
    pub fn exception_check(&self) -> bool {
        ...
        unsafe { jni_call_unchecked!(self, v1_2, ExceptionCheck) == JNI_TRUE }
    }
    ...
    pub unsafe fn new_direct_byte_buffer(
        ...
        self.ensure_version(JNI_VERSION_1_4)?;
